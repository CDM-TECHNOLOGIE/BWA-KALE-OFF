# ===== CONFIG =====
MOT_DE_PASSE = "ERROR404"   # <-- Change ici
...
║                                                                      ║
║{RED}{BOLD}   ╔══════════════════════════════════════════════════════════════╗
║{RED}{BOLD}   ║              🔥 A-FBI DEV : THE TERROR CDM 🔥                ║
║{RED}{BOLD}   ║                  [ SYSTEME FORTIFIÉ v2.0 ]                   ║
║{RED}{BOLD}   ║                       ⚡ ROBOOST ⚡                           ║
║{RED}{BOLD}   ╚══════════════════════════════════════════════════════════════╝
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝{RESET}
...
def typewriter(texte, couleur=CYAN, delai=0.015):  # Plus rapide (0.03 -> 0.015)
    ...
    for i in range(nombre_spam):
        try:
            ...
            while nombre_tentatives < tentatives_max:
                try:
                    typewriter(f"[*] Tentative de reconnexion... ({nombre_tentatives + 1}/{tentatives_max})", YELLOW, 0.01)  # Plus rapide
                    time.sleep(1.5)  # Plus rapide (2 -> 1.5)
                    msg = random.choice(messages)
                    envoyer_message(numero_telephone, msg)
                    typewriter("[+] Reconnecté avec succès !", GREEN, 0.01)  # Plus rapide
                    delai = random.uniform(*intervalle_delai)
                    time.sleep(delai)
                    ...
                    if nombre_tentatives == tentatives_max:
                        typewriter("[!] Nombre maximal de tentatives atteint. Message ignoré...", RED, 0.01)  # Plus rapide
                        time.sleep(intervalle_delai[1])
                        continue

def main():
    print_banner()
    typewriter("INITIALISATION DU SYSTÈME A-FBI DEV...", GREEN, 0.01)  # Plus rapide
    typewriter(f"{BOLD}DÉVELOPPEUR: THE TERROR CDM{RESET}", PURPLE, 0.015)  # Plus rapide
    time.sleep(0.8)  # Plus rapide (1 -> 0.8)
    ...
    confirmation = input(f"{PURPLE}╠══[A-FBI-503]> {RED}LANCER (Y/N): {RESET}").lower()
    
    if confirmation == 'o':
        typewriter("Hello", GREEN, 0.01)
        typewriter("Hello", YELLOW, 0.01)
        typewriter(f"{BOLD}Hello{RESET}", CYAN, 0.01)
        ...
        try:
            ...
            typewriter("Hello", RED, 0.01)
            typewriter("Hello", PURPLE, 0.01)
    else:
        typewriter("Hello", RED, 0.01)
